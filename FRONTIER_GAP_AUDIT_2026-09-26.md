# GreenLogix Frontier Plan — Implementation Verification & Gap Audit (EcoALNS v2 slice)

**Plan:** `GREENLOGIX_FRONTIER_PLAN_2026-09-1.md` (2026-09-21)
**Audit date:** 2026-09-26 (Asia/Ho_Chi_Minh)
**Branch:** `so2026`
**Method:** static code read + grep + `uv run pytest` from `apps/api` (**435 passed**, up from 429). Read-only reviewer gate PASS. No live Valhalla tiles / GOFA key / device test / OBD trials.
**Verdict:** 0/16 CRs fully pass. **16 PARTIAL, 0 NOT STARTED.** This iteration narrowed the CR-12/CR-13 v2 remainder at code level (SA acceptance + runtime budget + convergence trace + 2 GreenLogix destroy ops, all tested); CR-12/CR-13 keep remainders (PyVRP/OR-Tools refs, ban-window operator, payload-bin cache, restarts, full ablation), so no CR flips to PASS under the strict rule.

> Strict PASS rule: PASS only on full acceptance (no named remainder). DONE = wired + tested at code level. SCAFFOLD = honest stub with BLOCKED/infra note. Footnote on counts: 09-25 was 0/16/0 with feedback-admin closure DONE. This revision keeps 0/16/0 — one more §2 slice moved DONE (§1 T-ALNS-V2 + T-ALNS-TESTS) but parent CR-12/CR-13 retain named remainders.[^1]

[^1]: Prior files `FRONTIER_GAP_AUDIT_2026-09-21.md` / `2026-09-22.md` / `2026-09-23.md` / `2026-09-24.md` / `2026-09-25.md` remain untouched; this is the new dated revision.

**Contract:** `apps/api/openapi.json` holds **21 paths** (unchanged; this slice touches optimizer lib only, no router/schema edits; `test_frozen_openapi` green). Banned-claims grep: no truck-safe guarantee, no ISO-certified wording — matches are negations/wording-contract only (`carbon.py:16-19`, `routers/optimize.py:176`, `solver/flags.py:9`, `solver/road_baseline.py:30`). `.opencode-state/betriebsrat/vetoes/` empty.
...
| CR-12 | EcoALNS v1 + v2 operators | ⚠️ PARTIAL | 4 destroy + 2 repair + adaptive weights + **SA opt-in + budget + trace** + optimizer audit label; no PyVRP comparison, no ban-window op, no payload-bin cache, no restarts | `optimizer/eco_alns.py:131-335,344-586`; `carbon.py:104-119`; `tests/test_eco_alns.py`, `test_alns_operators.py` (10 tests), `test_optimizer_audit.py` |
| CR-13 | (operators — merged w/ CR-12) | ⚠️ PARTIAL | worst-fuel + uphill-payload destroy DONE; ban-window removal, eco-regret already done, payload-bin cache, restarts, full ablation missing | Same as CR-12 + `eco_alns.py:198-280` |
...
Strict rule applied: PASS only on full acceptance. One v2 slice closed at code level (SA + budget + trace + 2 ops), but CR-12/CR-13 retain named remainders — hence still 16× PARTIAL. (CR-12/CR-13 share the EcoALNS row; counted separately in the 16.)

---

## 1. Loop confirmations (09-26: T-ALNS-V2 + T-ALNS-TESTS)

| # | Item | State | Evidence |
|---|------|-------|----------|
| T-ALNS-V2 | SA acceptance + budget + trace + 2 GreenLogix destroy ops | ✅ DONE | `optimizer/eco_alns.py` `EcoSolution.convergence_trace` (:50-56, 7 keys/row); `solve_eco_alns` keyword-only `acceptance="hillclimb"\|"sa"`, `sa_temp0=1.0`, `sa_cooling=0.995`, `time_budget_s=None` (:400-433); closed-form temp (:492); SA uphill seeded-rng + unassigned-reject + no-reward (:526-545, :562-565); budget guard best-so-far (:476-480); `worst_fuel_removal_destroy` (:198-238) + `uphill_payload_removal_destroy` (:241-280) in `DESTROY_OPS` (:384-389) + adaptive weights (:469-474); legacy 3-arg fallback (:498-502); hill-climb default → pre-existing tests byte-behavioral |
| T-ALNS-TESTS | Tests + notes | ✅ DONE | `tests/test_alns_operators.py` 10 tests (4 existing unmodified + 6 new: SA determinism + cooling monotonic, SA no-worse-unassigned + capacity, budget best-so-far + TRACE_KEYS, worst_fuel deterministic + feasible, uphill deterministic + feasible, registration + adaptive 20-row trace); `implementation-notes.md` 2026-09-26 section appended; read-only reviewer gate PASS |

Test progression: 429 → **435 passed** (`apps/api`; +6 v2 tests). Targeted run `test_alns_operators.py + test_eco_alns.py`: 11 passed. Contract unchanged: **21 paths**; `test_frozen_openapi` green. No new deps (stdlib `time` only).

Limitation recorded: `CustomerNode` carries no grade field, so uphill proxy = carried-payload × leg-km (leg-km stands in for segment cost); grade-aware scoring waits for elevation/leg features. Uphill moves never earn adaptive reward (`improved`, not `accepted`).
...
5. **PostGIS admin polygons (1–2 d, NOT STARTED at infra level):** `LINESTRING × ward polygons`, versioned NSO dataset; endpoint shape exists so swap is contained.
6. **Valhalla tiles/Docker/elevation (3–5 d, NOT STARTED at infra level):** manifest carries dev placeholders (`unpinned-dev`); snapshot pin + tile build + `/health` reachability flip to True.
7. **Pareto Valhalla rerank Stage-1 (1–2 d):** alternatives → `hdt_v1` scoring → dominance filter; `source="verified"` path defined, unreachable until tiles land.
...
## 3. Demo honesty slice (26/09 — what can ship)
...
## 4. Credit (genuinely done 09-26)

- SA/record-to-record-adjacent acceptance opt-in (seeded, closed-form cooling, feasibility-guarded, no reward on uphill).
- Runtime budget opt-in returning best-so-far with convergence trace (7 keys/row, deterministic sans wall-clock).
- 2 GreenLogix destroy ops (worst-fuel marginal + uphill-payload carried-load proxy) registered + adaptive-weighted, all deterministic.
- **435 pytest green** (was 429: +6 v2 tests); contract unchanged (21 paths); banned-claims clean; vetoes empty; sequential-only, no parallel-agent violation.
...
- `apps/api/src/greenlogix_api/optimizer/eco_alns.py` (+~170: trace field, SA, budget, 2 destroy ops)
- `apps/api/tests/test_alns_operators.py` (+6 tests, 4 → 10)
- `implementation-notes.md` (2026-09-26 section appended)
- `FRONTIER_GAP_AUDIT_2026-09-26.md` (this file, new)
...
*End of audit 2026-09-26. Prior files `FRONTIER_GAP_AUDIT_2026-09-21.md` / `2026-09-22.md` / `2026-09-23.md` / `2026-09-24.md` / `2026-09-25.md` untouched. Next: §2 item 4 (ban-window destroy op + payload-bin cache + restarts + ablation rows) or land CR-001 to dev to unblock CR-002 rebase.*
