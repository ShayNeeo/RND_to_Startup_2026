# Olympic Khởi nghiệp 2026 (SO 2026) — Contest Packet

Official submission materials for the **Olympic Khởi nghiệp 2026** competition.

---

## 1. Competition Overview

- **Organizer**: Trường Đại học Kinh tế Quốc dân (NEU) & Trung tâm Đổi mới sáng tạo và Hướng nghiệp (CICN).
- **Track**: Dự án khởi nghiệp tạo tác động xã hội (Social Impact Business).
- **Current Phase**: Vòng 1 — Ý tưởng (Round 1: Idea).
- **Project Name**: **EcoMiles** (Phần mềm tối ưu tuyến đường vận tải đô thị và đo lường phát thải CO₂).
- **Live MVP Demo**: [https://greenlogix.w9.nu/](https://greenlogix.w9.nu/)

---

## 2. Founding Team Roster

| Member Name | University / Institution | Project Role | Domain Focus |
|---|---|---|---|
| **Nguyễn Thu Thuỷ** | Đại học Kinh tế Quốc dân (NEU) | International Business | Quản trị Kinh doanh Quốc tế & Chiến lược |
| **Phạm Quốc Thanh** | ĐH Quốc tế — ĐHQG TP.HCM (IU) | Fullstack & AI | Platform Engineering, Cloud Infrastructure & AI |
| **Nguyễn Ngọc Khánh Phương** | ĐH Ngoại Thương CS2 (FTU2) | Business Development | Marketing, ESG Partnerships & Stakeholder Relations |
| **Nguyễn Hồng Phúc** | Đại học FPT Hà Nội | Operations & Audit | Financial Projections, Cost Modeling & Internal Audit |
| **Lê Thị Hoàng Ngân** | Đại học Kinh tế Quốc dân (NEU) | E-commerce | Thương Mại Điện Tử & Vận Hành Kênh Số |

---

## 3. Document Manifest

| File | Type | Description |
|---|---|---|
| [`BÀI DỰ THI OLYMPIC KHỞI NGHIỆP 2026.docx`](BÀI%20DỰ%20THI%20OLYMPIC%20KHỞI%20NGHIỆP%202026.docx) | Word Document (1.05 MB) | Official 9-section contest submission paper covering problem analysis (17% logistics GDP cost, 80% road CO₂, 30-35% empty return trips), VRP routing engine, GLEC/GHG CO₂ measurement, business model canvas (BMC), financial projections, and ESG impact roadmap. |
| [`ĐƠN ĐĂNG KÝ VÀ CAM KẾT DỰ THI OLYMPIC KHỞI NGHIỆP 2026.docx`](ĐƠN%20ĐĂNG%20KÝ%20VÀ%20CAM%20KẾT%20DỰ%20THI%20OLYMPIC%20KHỞI%20NGHIỆP%202026.docx) | Word Document (628 KB) | Official registration form, team identity verification, and competition commitment agreement. |

---

## 4. Associated Presentation Deliverables

- **PowerPoint Deck**: [`slides/GreenLogix_Pitch_Deck_SO2026.pptx`](../../../slides/GreenLogix_Pitch_Deck_SO2026.pptx)
  - 10-slide narrative flow synchronized with teammate voiceover script (~157s).
  - High-Tech Dark design system with official NEU transparent logo (`docs/assets/logo_neu_transparent.png`) and circular team portraits (`docs/assets/team/`).
- **PDF Export**: [`slides/GreenLogix_Pitch_Deck_SO2026.pdf`](../../../slides/GreenLogix_Pitch_Deck_SO2026.pdf)
- **Slide Previews**: [`slides/previews/`](../../../slides/previews/)
