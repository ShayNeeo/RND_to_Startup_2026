...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("StatusOut", "status", ["arrived", "delivered", "failed"]),
...
def test_startup_creates_tables(isolated_database):
    with TestClient(app) as client:
        assert client.get("/health").json() == {"status": "ok"}
        assert set(inspect(isolated_database).get_table_names()) == {
            "orders",
            "vehicles",
            "routes",
            "stops",
...
def test_valid_patch_reaches_stub(monkeypatch, path, body):
    monkeypatch.setenv("GREENLOGIX_DEMO", "1")
    with TestClient(app) as client:
        response = client.patch(
            path, json=body, headers={"Authorization": "Bearer DEMO"}
...
def test_valid_status_reaches_stub(monkeypatch, body):
    monkeypatch.setenv("GREENLOGIX_DEMO", "1")
    with TestClient(app) as client:
        response = client.post(
            "/stops/1/status", json=body, headers={"X-Driver-Pin": "0000"}
...
@pytest.mark.parametrize(("field", "expected"), [("photo", 503), ("file", 422)])
def test_photo_field(monkeypatch, field, expected):
    monkeypatch.setenv("GREENLOGIX_DEMO", "1")
    with TestClient(app) as client:
        response = client.post(
            "/stops/1/photo",
            files={field: ("proof.jpg", b"stub upload", "image/jpeg")},
